 ...
 * scrollarax
 *
 * https://github.com/Prinzhorn/scrollarax
 *
...
(function(window, document, undefined) {
	 ...
	 * Global api.
	 */
	var scrollarax = window.scrollarax = {
		get: function() {
			return _instance;
		},
		//Main entry point.
		init: function(options) {
			return _instance || new Scrollarax(options);
		},
		VERSION: '0.5.14'
	};
	...
	//They will be filled when scrollarax gets initialized.
	var documentElement;
	var body;
	...
	//Vendor prefix. Will be set once scrollarax gets initialized.
	var theCSSPrefix;
	var theDashedCSSPrefix;

	//Will be called once (when scrollarax gets initialized).
	...
	function Scrollarax(options) {
		...
		//Remove "no-scrollarax" and add "scrollarax" to the HTML element.
		_updateClass(documentElement, [SKROLLR_CLASS], [NO_SKROLLR_CLASS]);

		if(_forceHeight) {
			//Add a dummy element in order to get a large enough scrollbar.
			//On mobile and later desktop versions a #scrollarax-body element takes this role.

			var dummy = document.getElementById('scrollarax-body') || document.createElement('div');
		...
		scrollarax.addEvent(window, 'resize', _reflow);
	 ...
	 */
	Scrollarax.prototype.refresh = function(elements) {
		var elementIndex;
		var elementsLength;
		var ignoreID = false;
	 ...
	 */
	Scrollarax.prototype.relativeToAbsolute = function(element, viewportAnchor, elementAnchor) {
	 ...
	 */
	Scrollarax.prototype.animateTo = function(top, options) {
		options = options || {};
	 ...
	 */
	Scrollarax.prototype.stopAnimateTo = function() {
	 ...
	 */
	Scrollarax.prototype.isAnimatingTo = function() {
		return !!_scrollAnimation;
	};

	Scrollarax.prototype.setScrollTop = function(top, force) {
		...
		//scrollarax.iscroll is an instance of iscroll available in mobile mode
		if(scrollarax.iscroll) {
			//Notice the minus.
			scrollarax.iscroll.scrollTo(0, -top);
		} else {
	...
	Scrollarax.prototype.getScrollTop = function() {
		//scrollarax.iscroll is an instance of iscroll available in mobile mode
		if(scrollarax.iscroll) {
			return -scrollarax.iscroll.y;
		} else {
			return window.pageYOffset || documentElement.scrollTop || body.scrollTop || 0;
		}
	};

	Scrollarax.prototype.on = function(name, fn) {
		_listeners[name] = fn;

		return _instance;
	};

	Scrollarax.prototype.off = function(name) {
	...
	*/
		...
		for(; skrollableIndex < skrollablesLength; skrollableIndex++) {
			...
			for(; keyFrameIndex < keyFramesLength; keyFrameIndex++) {
				...
				if(kf.mode === 'relative') {
					...
					//Now set style and class back to what scrollarax did to it.
			...
			if(atFirst || atLast) {
				...
				for(key in props) {
					if(hasProp.call(props, key)) {
						...
						scrollarax.setStyle(skrollable.element, key, value);
					}
				}
	 ...
	 */
	scrollarax.setStyle = function(el, prop, val) {
		var style = el.style;
	 ...
	 */
	scrollarax.addEvent = function(element, name, callback) {
	...
	/*
		...
		skrollable = {
			...
			styleAttr: <style attribute of the element before scrollarax>,
			classAttr: <class attribute of the element before scrollarax>,
			keyFrames: [
				{
					frame: 100,
					props: {
